...
const AccountSettings = ({ applyUserFilter }) => {
          ...
          firebase
            ...
            .then(async (doc) => {
              if (!doc.exists) {
                ...
                await SpritjaktClient.CreateUserDoc(
          ...
          .onSnapshot(async (doc) => {
            ...
            if (userData) {
              ...
              if (userData.products && userData.products.length > 0) {
                setIsLoading(true);
                SpritjaktClient.FetchProductsById(userData.products).then(
                  (products) => {
                    setIsLoading(false);
    ...
    if (user.providerData[0].providerId !== "password") {
      if (pass !== "SLETT") {
        notification.current.setNotification(
          event,
          "Skriv SLETT for å slette brukeren din.",
          "error"
        );
        return;
      }
      ...
      firebase
        ...
        .catch(() => {
          notification.current.setNotification(
            event,
            "Noe gikk galt, det kan hende du må tillate pop-ups.",
            "error"
          );
        });
    } else {
  ...
  const deleteUserDoc = async (userId, event) => {
    ...
    user
      ...
      .catch(function (error) {
        alert(
          "Noe gikk galt, brukeren ble ikke slettet. Feilmelding: " +
            error.message
        );
      });
  };
  ...
  const renderFilters = () => {
    ...
    for (const filter of userData.filters) {
      ...
      items.push(
        <li key={userData.filters.indexOf(filter)} className="filter">
          <div className="operations">
            <button
              aria-label={
                "Velg filter. Butikker: " +
                storeNames +
                ". Produkttyper: " +
                filter.productTypes.join() +
                ". Land: " +
          ...
          <div className="details">
            <div className="stores">
              {storeNames.length > 0
                ? storeNames.join().replace(/,/g, ", ")
                : "Alle"}
            </div>
            <div className="productTypes">
              {filter.productTypes.length > 0
                ? filter.productTypes.join().replace(/,/g, ", ")
                : "Alle"}
            </div>
            <div className="countries">
              {filter.countries?.length > 0
                ? filter.countries?.join().replace(/,/g, ", ")
                : "Alle"}
            </div>
            <div className="isGoodForList">
              {filter.isGoodFor?.length > 0
                ? filter.isGoodFor?.join().replace(/,/g, ", ")
                : "Alle"}
            </div>
            <div className="pris">{`${filter.min || ""} - ${
              filter.max || ""
            }`}</div>
          </div>
          <div className="operations">
  ...
  const nextProduct = (change) => {
    ...
    let newHighlightedProduct =
      productResult[highlightedProductIndex + change] ?? null;
  ...
  const highlightProduct = async (productId) => {
    if (productId === null || productId === highlightedProduct?.Id) {
      ...
      let product =
        productResult.find((p) => p.Id === productId) ||
        (await SpritjaktClient.FetchProductById(productId));
      ...
      {userData && (
        <div className="profileStatusBar">
          <button
            ...
            name="Togglesettings"
            onClick={() => toggleSection(!isActive)}
            className="iconBtn toggleSettings"
          >
          ...
          {showContent && (
            <div>
              <div className="sectionHeader toolbar">
                <button
                  aria-label="Skjul innstillinger"
                  name="Togglesettings"
                  onClick={() => toggleSection(!isActive)}
                  className="iconBtn toggleSettings"
                >
                ...
                <button
                  name="logout"
                  onClick={logout}
                  className="bigGreenBtn clickable logout"
                >
                  Logg ut
                </button>
              </div>
              ...
              <div className="favorites">
                ...
                {userData.products &&
                  (productResult.length < 1 || productResult === undefined) && (
              ...
              <div className="heading">
                ...
                {nameChange ? (
                  <div className="changeName">
                    ...
                    />
                    <button
                      className="bigWhiteBtn clickable"
                      onClick={() => {
                        setNameChange(false);
                      }}
                    >
                    ...
                    <button
                      ...
                      onClick={changeName}
                      className="bigGreenBtn clickable"
                    >
                      Lagre
                    </button>
                  </div>
                  ...
                  <div className="changeName">
                    ...
                    <button
                      ...
                      className="iconBtn clickable dark"
                      onClick={() => {
                        setNameChange(true);
                      }}
                    >
                ...
                <label>
                  <input
                    type="checkbox"
                    checked={permissions.onAll}
                    onChange={handleNotifications}
                    name="onAll"
                  />{" "}
                  Ved alle prisendringer
                </label>
                <br />
                <label>
                  <input
                    type="checkbox"
                    onChange={handleNotifications}
                    checked={permissions.onFilters}
                    name="onFilters"
                  />{" "}
                  Ved prisendringer i lagrede filtre
                </label>
                <br />
                <label>
                  <input
                    type="checkbox"
                    onChange={handleNotifications}
                    checked={permissions.onFavorites}
                    name="onFavorites"
                  />{" "}
                  Ved prisendringer i favoritter
                </label>
                <br />
                  ...
                  <div className="deleteWarning">
                    <form onSubmit={deleteUser}>
                      ...
                      user?.providerData?.length > 0 ? (
                        <label>
                          {user.providerData[0].providerId !== "password" ? (
                            <span>
                              ...
                              <input
                                type="text"
                                aria-label="Skriv SLETT for å bekrefte sletting"
                                name="password"
                              />
                            </span>
                          ) : (
                            <span>
                              ...
                              <input
                                type="password"
                                aria-label="Oppgi passord for å bekrefte sletting"
                                name="password"
                              />
                            </span>
                          )}
                        ...
                        <span>
                          Lagrede filtre og favoritter vil slettes, og du vil
                          ikke lenger kunne logge inn med denne kontoen.
                          <br />
                      ...
                      {deleteProcessStarted && (
                        <button
                          className="clickable bigWhiteBtn"
                          onClick={() => {
                            setDeleteProcess(false);
                          }}
                        >
