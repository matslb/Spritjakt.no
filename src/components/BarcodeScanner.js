...
const BarcodeScanner = () => {
  ...
  const updateResult = async (result) => {
    ...
    if (id) {
      ...
      if (product != undefined) {
        ...
        notificationRef.current.setNotification(
          null,
          "Jeg fant, jeg fant!",
          "success"
        );
  ...
  const debouncedChangeHandler = useCallback(debounce(updateResult, 250), [
    updateResult,
    250,
  ]);
          ...
          <div ref={scannerRef} className="VideoWrapper">
            {scanning && (
              <button
                ...
                onClick={openBarcodeScanner}
                className={
                  "clickable iconBtn torch " + (useTorch ? "active" : "")
                }
              >
                <FontAwesomeIcon
                  size="2x"
                  onClick={() => setUseTorch(!useTorch)}
                  icon={faLightbulb}
                />
              </button>
            )}
            {scanning && (
              <Scanner
                ...
                onDetected={debouncedChangeHandler}
              />
            )}
          </div>
          {scanning && (
      ...
      {product != null && (
        <ProductPopUp
          ...
          highlightProduct={async (productId) => {
            if (productId == null) setProduct(null);
