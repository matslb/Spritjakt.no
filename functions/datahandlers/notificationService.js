...
module.exports = class NotificationClient {
    ...
    static CreateFilterEmail(userFilterMatch) {
        ...
        urlParams += userFilterMatch.filter.countries && userFilterMatch.filter.countries.length > 0 ? "&countries=" + userFilterMatch.filter.countries.join() : "";

        let greeting = userFilterMatch.user.name ? "Hei " + userFilterMatch.user.name : greetings[Math.floor(Math.random() * greetings.length)];
        let subject = "Ditt lagrede filter har fått " + userFilterMatch.products.length + (userFilterMatch.products.length > 1 ? " nye" : " nytt") + " tilbud!";
        let subheader = subject;
        let email = this.CreateEmail(greeting, subheader, userFilterMatch.products, urlParams);
    ...
    static async SendAllPush(user, count) {
        ...
        let message = {
            notification: {
                ...
                body: "Det er " + count + (count > 1 ? " nye" : " nytt") + " tilbud i dag, og det er jo artig!",
    ...
    static async SendFilterPush(userFilterMatch) {
        let urlParams = userFilterMatch.filter.productTypes.length > 0 ? "types=" + userFilterMatch.filter.productTypes.join() : "";
        urlParams += userFilterMatch.filter.stores.length > 0 ? "&stores=" + userFilterMatch.filter.stores.join() : "";

        let message = {
            notification: {
                title: "Filteret ditt har fått " + userFilterMatch.products.length + (userFilterMatch.products.length > 1 ? " nye" : " nytt") + " tilbud!",
    ...
    static async SendFavoritesPush(userFavoriteMatch) {
        ...
        await userFavoriteMatch.products.forEach(async p => {
            ...
            message.notification.body = "En endring på " + (p.PriceChange - 100).toFixed(1) + "%";
    ...
    static addProductListTextBlock(products) {
        ...
        for (const i in products) {
            ...
            text += "\n--------------------------\n"
                + product.Name + " - " + product.Types.join()
                + "\n\nNy pris: kr " + product.LatestPrice
                + "\nGammel pris: kr " + this.getComparingPrice(product)
                + "\nPrisendring: " + (product.PriceChange - 100).toFixed(1) + "%"
                + "\n--------------------------\n"
        }
        return text;
    }
    ...
    static addProductListBlock(products, urlParams) {
        ...
        for (const i in products) {
            ...
            productItem = productItem.replace(/&OldPrice&/g, this.getComparingPrice(product));
            productItem = productItem.replace(/&Discount&/g, (product.PriceChange - 100).toFixed(1));
            productItem = productItem.replace(/&ProductImageLink&/g, "https://bilder.vinmonopolet.no/cache/100x100/" + product.Id + "-1.jpg");
            productItem = productItem.replace(/&ProductLink&/g, "https://spritjakt.no/?source=email&product=" + product.Id + urlParams);
            productItem = productItem.replace(/&ProductDescription&/g, product.Types.join());
            html += productItem;
        }
        return html;
    }
