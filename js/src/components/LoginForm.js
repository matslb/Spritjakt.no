...
const LoginForm = ({ formType, setFormType }) => {
  ...
  const register = (event) => {
    ...
    if (pass.length <= 5) {
      setErrorMessage({
        status: false,
        message: "Passordet må være på minst 6 tegn",
      });
      return;
    }
    if (pass !== pass2) {
  ...
  const resetPassword = (event) => {
    ...
    setErrorMessage({
      status: true,
      message:
        "Vi har send deg en link på e-post hvor du kan tilbakestille passordet ditt.",
    });
  };
  ...
  return (
    <div>
      {formType === formTypes.resetPass ? (
        ...
        <form
          className="loginForm"
          onSubmit={formType === formTypes.login ? login : register}
        >
          <h2>{formType}</h2>
          <label>
            E-post
          ...
          {formType === formTypes.register && <NotificationSection />}
          <br />
          <input
      ...
      {errorMessage.status !== null && (
        <div
          className={
            "statusMessage" + (errorMessage.status ? " success" : " error")
          }
        >
          {!errorMessage.status ? (
