...
class MainContent extends React.Component {
  ...
  getQuery = () => {
    ...
    query.stores = cleanForMissingStores(
      toArray(query.stores),
      this.state.stores
    );
  ...
  setFacetCount = (selected, facetcounts) => {
    ...
    for (const f of facetcounts) {
      ...
      else selected[index].count = f.count;
    }
    return selected;
  };
  ...
  async fetchProducts(query = this.getQuery()) {
    ...
    let result = await this.typeSenseClient.fetchProducts(
      query,
      this.state.pageSize
    );
    ...
    for (const f of result.facet_counts) {
      switch (f.field_name) {
        case "Country":
          var productCountries = this.setFacetCount(
            this.state.productCountries,
            f.counts
          );
          break;
        case "Types":
          var productTypes = this.setFacetCount(
  ...
  nextProduct = (change) => {
    let highlightedProductIndex = this.state.productResult.indexOf(
      this.state.highlightedProduct
    );
    let newHighlightedProduct =
      this.state.productResult[highlightedProductIndex + change] ?? null;
  ...
  highlightProduct = async (productId) => {
    if (productId === null) {
      let productButton = document.querySelector(
        "#p-" + this.state.highlightedProduct.Id + "> button"
      );
      if (productButton) {
        if (!isInViewport(productButton))
          window.scroll({
            top:
              productButton.getBoundingClientRect().top +
      ...
      <React.Fragment key={anchor}>
        ...
        <div className="nav">
          <SearchBar
            ...
            searchIsActive={isSearch}
            loading={loading}
            searchStringProp={this.state.searchString}
            forceSearchString={this.state.forceSearchString}
          />
          <FilterV2
            items={isGoodFor}
            selectedItems={filter.isGoodFor}
            propSlug={"isGoodFor"}
            label={"Passer til"}
            handleFilterChange={this.handleFilterClick.bind(this)}
          />
          <FilterV2
            items={productTypes}
    ...
    return (
      <div key="MainContent" className="MainContent">
        <div className="main">
          <main>
            {!isMobile && (
              <div className="before-products">{this.getFilterSection()}</div>
            )}
            {isMobile && (
              <React.Fragment key={"drawer"}>
                ...
                >
                  <Card>
                    ...
                    <CardContent>{this.getFilterSection()}</CardContent>
                  </Card>
                </SwipeableDrawer>
              </React.Fragment>
            )}
